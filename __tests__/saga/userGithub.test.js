...
describe("Testing User Github SAGA", () => {
  ...
  it("can add user", async () => {
    apiMock
      .onGet("/users/GabrieldOliveira")
      .reply(200, userGithubFixture["/users/GabrieldOliveira"]);

    sagaTester.dispatch(actions.addUserRequest("GabrieldOliveira"));

    await sagaTester.waitFor(actions.addUserSuccess().type);

    expect(sagaTester.getLatestCalledAction()).toEqual(
      actions.addUserSuccess(userGithubFixture["/users/GabrieldOliveira"])
    );
  });
